//Code taken from https://github.com/supertone-inc/supertonic/blob/main/rust/src/helper.rs
...
pub fn _write_wav_file<P: AsRef<Path>>(
    filename: P,
    audio_data: &[f32],
    sample_rate: i32,
) -> Result<()> {
...
pub fn _timer<F, T>(name: &str, f: F) -> Result<T>
where
    F: FnOnce() -> Result<T>,
{
...
pub fn _sanitize_filename(text: &str, max_len: usize) -> String {
